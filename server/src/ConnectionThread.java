...
public class ConnectionThread extends Thread {
   ...
   * @param s
   */
  public ConnectionThread(SSLServerSocket server, SSLSocket client) {
   ...
   * @param message Message to send.
   */
  public void messageAllSockets(String connector, String message, int flag) {
    for(ConnectionThread ct : allConnections) {
    	if(flag==0) {
    		ct.messageSocket("Connection " + String.valueOf(this.connectionNumber), connector, message);
    		if(ct.connectionNumber==this.connectionNumber) {
    			for(ConnectionThread ct2 : allConnections) {
    				ct.messageSocket("Connection " + String.valueOf(ct2.connectionNumber), " is", " available !!!!");
    			}
    		}
